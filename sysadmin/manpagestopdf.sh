...
SYSADMIN_COMMANDS="addgroup cd mv cp rm du df dfc rename touch delgroup adduser deluser groupadd groupdel useradd userdel dmsetup taskset insserv rcconf update-rc.d at lastb ls w who wtmp chage chmod chown chroot df dmesg getfacl setfacl install lsof lspci lsusb lshw inxi mkfs fsck ntfsfix proc ps pstree top uptime vmstat signal xargs faillog mktemp ltrace find locate update-alternatives pgrep bash htop fuser kill pkill killall atool tar gzip zip unrar 7z zcat zless logrotate ranger mount mdadm lvm 7_signal pwd which service chattr logsave readlink realpath free iostat iotop sar mpstat pmap strace proc atop powertop whoami who id finger pinky multitail swatch tmux screen wall cryptsetup sudo su lsblk lynis rkhunter chkrootkit tiger maldet fdisk sfdisk parted stat dd uname mkdir passwd shells chsh env printenv"

TEXTHANDLING_COMMANDS="awk cat cut grep head tail echo less sed tr wc ascii cmp comm column dirname basename fmt strings diff wdiff paste nl split sort shuf tee sponge readlink nl cmp"

MISC_COMMANDS="apropos whereis man cheat inotifywait xclipboard xclip hier watch wmctrl yes pdfimages enscript ps2pdf chm2pdf pdfimages pdftotext pdfunite date xdg-open svn git pv gnupg shuff dialog whiptail tzselect notify-send elinks readelf objdump md5sum alias echo"
