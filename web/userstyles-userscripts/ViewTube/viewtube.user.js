...
// @downloadURL		http://isebaro.com/viewtube/include/files/viewtube.user.js
// @updateURL		http://isebaro.com/viewtube/include/files/viewtube.user.js
...
// Player
var player = {};
var feature = {'autoplay': true, 'definition': true, 'container': true, 'dash': false, 'widesize': true, 'fullsize': true};
var option = {'plugin': 'Auto', 'autoplay': false, 'autoget': false, 'definition': 'HD', 'container': 'MP4', 'dash': false, 'widesize': false, 'fullsize': false};
...
function createMyPlayer () {
  ...
  /* Panel DASH Button */
  if (feature['dash']) {
    player['buttonDASH'] = createMyElement ('div', 'DASH', 'click', 'dash', '');
...
page.win.setInterval(function() {
  ...
  if (page.url != nurl) {
    ...
    // Facebook
      ...
      if (nurl.indexOf('facebook.com/video.php?v=') != -1) {
...
if (page.url.indexOf('youtube.com/watch') != -1) {
  ...
  if (!ytPlayerWindow) {
	...
	ytVideoID = (ytVideoID) ? ytVideoID[2] : null;
      ...
      if (ytVideoID) {
	var ytVideosInfoURL = page.win.location.protocol + '//' + page.win.location.hostname + '/get_video_info?video_id=' + ytVideoID;
    ...
    var ytPlayerHeight = 382;
    var ytPlayerWideWidth = 1066;
    var ytPlayerWideHeight = 622;
    var ytSidebarMarginWide = 10;
    var ytScreenWidth = page.win.innerWidth || page.doc.documentElement.clientWidth;
    var ytScreenHeight = page.win.innerHeight || page.doc.documentElement.clientHeight;
...
// =====DailyMotion===== //
  ...
  if (!dmPlayerWindow) {
    ...
    if (dmContent) styleMyElement(dmContent, {maxWidth: '1000px'})
	...
	if (dmVideo) {
      ...
      if (dmVideoFound) {
	/* Get Watch Sidebar */
	var dmSidebarWindow = getMyElement ('', 'div', 'class', 'js-related-box', 0, false);
	...
	var dmPlayerContainer = getMyElement ('', 'div', 'id', 'player_container', -1, false);
	if (dmPlayerContainer) {
	  page.win.setTimeout(function() {
	    styleMyElement(dmPlayerContainer, {height: player['playerWindow'].style.height});
	  }, 1000);
...
else if (page.url.indexOf('facebook.com/video') != -1) {
